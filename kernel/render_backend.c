...
void graphics_putpixel(int x, int y, uint8_t color) {
    if (window_transform_active) {
        x += window_offset_x;
        y += window_offset_y;
        if (x < window_clip_x || x >= window_clip_x + window_override_w || y < window_clip_y || y >= window_clip_y + window_override_h) {
            return;
        }
    }
...
void graphics_draw_window(int x, int y, int w, int h, const char *title, bool active) {
    ...
    if (draw_h >= 16) {
        graphics_rect(x + 1, y + 1, draw_w - 2, 12, draw_active ? 1 : 8);
        graphics_text(x + 4, y + 3, title, 15);
        ...
        if (draw_w >= 42) {
            ...
            graphics_rect(min_x, by, 10, 10, 7);
            graphics_box(min_x, by, 10, 10, 15);
            graphics_text(min_x + 3, by + 2, "_", 15);

            graphics_rect(max_x, by, 10, 10, 7);
            graphics_box(max_x, by, 10, 10, 15);
            graphics_text(max_x + 2, by + 2, "O", 15);

            graphics_rect(close_x, by, 10, 10, 4);
            graphics_box(close_x, by, 10, 10, 15);
            graphics_text(close_x + 2, by + 2, "X", 15);
        }

        if (draw_w >= 20 && draw_h >= 24) {
            int grip_x = x + draw_w - 8;
            int grip_y = y + draw_h - 8;
            graphics_putpixel(grip_x + 4, grip_y + 6, 15);
            graphics_putpixel(grip_x + 5, grip_y + 5, 15);
            graphics_putpixel(grip_x + 6, grip_y + 4, 15);
            graphics_putpixel(grip_x + 2, grip_y + 6, 8);
            graphics_putpixel(grip_x + 3, grip_y + 5, 8);
            graphics_putpixel(grip_x + 4, grip_y + 4, 8);
            graphics_putpixel(grip_x + 5, grip_y + 3, 8);
            graphics_putpixel(grip_x + 6, grip_y + 2, 8);
        }
    }
}
...
void graphics_draw_chrome_window_shell(int x, int y, int w, int h, const char *title, int menu_bar_x, int menu_bar_y, int menu_bar_w, bool active) {
    graphics_draw_window(x, y, w, h, title, active);
    graphics_rect(menu_bar_x, menu_bar_y, menu_bar_w, 12, 7);
    graphics_box(menu_bar_x, menu_bar_y, menu_bar_w, 12, 15);
    graphics_rect(menu_bar_x + 1, menu_bar_y + 12, menu_bar_w - 2, 1, 8);
}

void graphics_draw_chrome_menu_title(int x, int y, int highlight_w, const char *label, bool selected) {
    if (selected) {
        graphics_rect(x - 2, y - 2, highlight_w, 10, 8);
    }
    graphics_text(x, y, label, selected ? 14 : 15);
}

void graphics_draw_chrome_menu_dropdown_frame(int x, int y, int w, int h) {
    graphics_rect(x, y, w, h, 7);
    graphics_box(x, y, w, h, 15);
}

void graphics_draw_chrome_menu_dropdown_item(int x, int y, int w, const char *label, bool selected) {
    if (selected) {
        graphics_rect(x + 2, y, w - 4, 10, 8);
    }
    graphics_text(x + 6, y + 1, label, selected ? 14 : 15);
}
